...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.view(num_train, -1)
    x_test_flat = x_test.view(num_test, -1)
    test_sq_norms = torch.sum(x_test_flat**2,dim = 1)

    for i in range(num_train):
        train_sample = x_train_flat[i]
        train_sq_norm = torch.sum(train_sample ** 2)

        dot_products = torch.matmul(x_test_flat,train_sample)
        dists[i] =  train_sq_norm + test_sq_norms - 2 * dot_products
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flat = x_test.view(num_test,-1)
    test_sq_norms = torch.sum(x_test_flat ** 2, dim=1)
    train_sq_norms = torch.sum(x_train_flat ** 2 , dim = 1)
    dot_products = torch.mm(x_test_flat, x_train_flat.t())
    dists = train_sq_norms.view(-1, 1) + test_sq_norms.view(1, -1) - 2 * dot_products.t()
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
    # dists[:, j] 形状为 (num_train,)，表示所有训练样本到第j个测试样本的距离
        values,indices = torch.topk(dists[:,j],k,largest=False)
        k_nearest_labels = y_train[indices]
        labels,counts = torch.unique(k_nearest_labels,return_counts = True)

        max_count = torch.max(counts)
        candidate_labels = labels[counts == max_count]

    # 如果有多个标签出现次数相同，选择最小的标签
...
def knn_cross_validate(
    ...
    Returns:
        k_to_accuracies: Dictionary mapping values of k to lists, where
            `KnnClassifier` that uses k nearest neighbors.    k_to_accuracies[k][i] is the accuracy on the i-th fold of a
    ...
    x_train_folds = list(x_train_folds)
    y_train_folds = list(y_train_folds)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            for j in range(num_folds):
                if j != i :
                    x_train_current.append(x_train_folds[j])
                    y_train_current.append(y_train_folds[j])
            x_train_combined = torch.cat(x_train_current, dim=0)
            y_train_combined = torch.cat(y_train_current, dim=0)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k , accuracies in k_to_accuracies.items():
        ...
        if mean_accuracy > best_mean_accuracy or (abs(mean_accuracy-best_mean_accuracy))< 1e-6 and k < best_k:
