...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        X_reshaped = x.reshape(N, -1)  # reshape input to (N, D)
        out = X_reshaped.mm(w) + b  # (N, M) out = x · w + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N = x.shape[0]
        X_reshaped = x.reshape(N, -1)  # (N, D) where D = d1 * ... * d_k
        dx_reshaped = dout.mm(w.t())  # (N, D) dx = dout · ∂y/∂x = dout · w^T
        dx = dx_reshaped.reshape(x.shape)  # reshape back to (N, d1, ..., d_k)
        dw = X_reshaped.t().mm(dout)  # (D, M) dw = ∂L/∂w = (∂L/∂y) · (∂y/∂w) = x^T · dout
        db = dout.sum(dim=0)  # (M,) db = ∂L/∂b = sum over all samples of dout
        '''examination:前向：
x: (N, D) × w: (D, M) + b: (M,) → y: (N, M)

反向：
dx: dout(N, M) × w^T(M, D) → (N, D) → reshape → (N, d1,...,dk)
dw: x^T(D, N) × dout(N, M) → (D, M)
db: dout(N, M) sum(dim=0) → (M,)'''
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code
        dx = dout.clone() # make a copy of dout
        dx[x <= 0] = 0 # zero out gradients where x was non-positive f'(x) = 1  if x > 0;f'(x)= 0  if x ≤ 0
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_loss = self.reg * (torch.sum(self.params['W1']**2) + torch.sum(self.params['W2']**2)) # L2 regularization loss, no 0.5 factor
        loss = data_loss + reg_loss
        # Backward pass
        dx2, dW2, db2 = Linear.backward(dscores, cache2)
        dx1, dW1, db1 = Linear_ReLU.backward(dx2, cache1)
        # Store gradients
        grads['W2'] = dW2 + 2 * self.reg * self.params['W2'] # add reg gradient 总梯度 = 数据梯度 + 正则化梯度
        grads['b2'] = db2 
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        layer_dims = [input_dim] + hidden_dims + [num_classes]  # all layer dimensions
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        out = X
        caches = [] # store caches for each layer
        dropout_caches = [] # store dropout caches
        # Forward pass through L-1 layers with ReLU (and dropout if used)
        ...
        scores, final_cache = Linear.forward(out, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        ...
        for i in range(self.num_layers):
            ...
            reg_loss += 0.5 * self.reg * torch.sum(W**2)  # L2 regularization loss with 0.5 factor
        loss = data_loss + reg_loss
        # Backward pass
        dout, dW, db = Linear.backward(dscores, final_cache)
        grads['W' + str(self.num_layers)] = dW + self.reg * self.params['W' + str(self.num_layers)]
        grads['b' + str(self.num_layers)] = db
        # Backward pass through L-1 layers
        for i in reversed(range(self.num_layers - 1)):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_caches[i])
            # Backprop through Linear-ReLU layer
...
def sgd_momentum(w, dw, config=None):
    ...
    v = mu * v - learning_rate * dw  # update velocity # v = mu * v + (- learning_rate * dw)
    next_w = w + v  # update weights # w = w + v
...
def rmsprop(w, dw, config=None):
    ...
    cache = config['cache']
    cache = decay_rate * cache + (1 - decay_rate) * (dw ** 2)  # update cache
    next_w = w - learning_rate * dw / (torch.sqrt(cache) + epsilon)  # update weights
    config['cache'] = cache
...
def adam(w, dw, config=None):
    ...
    next_w = w - learning_rate * m_hat / (torch.sqrt(v_hat) + epsilon)  # update weights
    config['m'] = m
    config['v'] = v
    config['t'] = t
