...
def svm_loss_vectorized(
    ...
    margins = torch.clamp(
        scores - correct_class_scores + 1, min=0
    )  # (N, C), max(0, s_j - s_yi
    margins[torch.arange(num_train), y] = 0  # do not consider j = yi
...
def test_one_param_set(
    ...
    cls.train(
        data_dict["X_train"], data_dict["y_train"], lr, reg, num_iters
    )
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        probs = exp_scores / sum_exp_scores  # (C,)
